...
public class PracticaWindow extends JFrame {
	...
	public PracticaWindow(Curso cursoA, Estudiante est) {
		...
		ProgresoEstudiante tmp = progDAO.buscarPorEstudianteYCurso(estudiante, curso);
		if (tmp == null) {
			tmp = new ProgresoEstudiante(estudiante, curso);
			progDAO.guardar(tmp);
		}
		this.progreso = tmp;
		...
		case ALEATORIO  -> new EstrategiaAleatoria();
	...
	private void mostrarSiguiente() {
		...
		contenedor.add(new FlashcardWindow(f) {
			...
			@Override protected void onRespuesta(boolean ok) {
				...
				progreso.registrarRespuesta(f, ok);
				progDAO.guardar(progreso);
				actualizarBarra();
				new Timer(1500, e -> mostrarSiguiente()) {
					/**
					 * 
					 */
					private static final long serialVersionUID = 1L;
