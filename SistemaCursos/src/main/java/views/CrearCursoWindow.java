...
public class CrearCursoWindow extends JFrame {
	...
	private void initUI() {
		...
		datos.add(new JLabel("Visibilidad:"), g);
		publicoCheckBox = new JCheckBox("Curso público (visible para todos)");
		publicoCheckBox.setSelected(true); // Por defecto público
		g.gridx = 1;
		datos.add(publicoCheckBox, g);
	...
	private void cargarCurso() {
		...
		publicoCheckBox.setSelected(cursoExist.isPublico());
	...
	private void guardarCurso() {
		...
		Set<Flashcard> fc = new HashSet<>();
		for (int i = 0; i < modelFlash.size(); i++) fc.add(modelFlash.get(i));
		
		boolean esPublico = publicoCheckBox.isSelected();
		...
		try {
			if (cursoExist == null) {
				...
				cursoExist.setTipoEstrategia(
						(TipoEstrategia) estrategiaCombo.getSelectedItem());
				cursoExist.setPublico(esPublico);
				cursoExist.getFlashcards().clear();
				
				for (Flashcard flashcard : fc) {
	                cursoExist.addFlashcard(flashcard); // Usar el método del dominio
	            }
